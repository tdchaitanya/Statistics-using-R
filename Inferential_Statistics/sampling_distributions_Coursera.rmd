...
ggplot(data = samp1 ,aes(x=area))+
  geom_histogram(binwidth = 250)
samp1 %>% summarise(sample_mu = mean(area), sample_median = median(area),
                    sample_sigma = sd(area) ,sample_iqr = IQR(area),
                    sample_min = min(area) , samplle_max = max(area),
                    sample_q1 = quantile(area,0.25),sample_q3 = quantile(area,0.75))
```
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
sample_means50 %>% summarise(mean_sample = mean(x_bar))
```
...
sample_means50 <- ames %>%
  ...
  summarise(x_price_bar = mean(price))

ggplot(data = sample_means50 ,aes(x = x_price_bar)) + 
...
sample_means50 %>% 
  summarise(std_50 = sd(x_price_bar))
sample_means150 %>% 
  summarise(std_150 = sd(x_bar))
```
