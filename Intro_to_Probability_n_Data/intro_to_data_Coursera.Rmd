...
sfo_feb_flights %>%
  summarise(mean_dd = mean(arr_delay),med = median(arr_delay), sd_dd = sd(arr_delay), variance = var(arr_delay),iqr = IQR(arr_delay),minimum = min(arr_delay),maximum = max(arr_delay),n = n())
...
nycflights %>%
  group_by(month) %>%
  summarize(med_max =median(dep_delay)) %>%
  arrange(desc(med_max))
```
...
nycflights <- nycflights %>%
  mutate(dep_type = ifelse(dep_time < 5, "on time", "delayed"))
nycflights %>%
  group_by(origin) %>%
  summarise(ot_dep_rate = sum(dep_type == "delayed") / n()) %>%
  arrange(desc(ot_dep_rate))
...
which.max(nycflights$avg_speed)
nycflights$tailnum[5067]
```
